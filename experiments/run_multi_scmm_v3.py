...
from src.simulator.scmm_v3_simulator import SCMMV3Simulator
from src.utils.helpers import ensure_dir


def run_one(seed: int) -> dict:
    params = Params(seed=seed)
    sim = SCMMV3Simulator(params)
    df = sim.run()
...
def save_histogram(df_runs: pd.DataFrame, plot_dir: Path) -> None:
    ...
    plt.title("Distribution of Final Wealth Across SCMM-v3 Runs")
    plt.tight_layout()
    plt.savefig(plot_dir / "multi_run_scmm_v3_final_wealth_hist.png")
    plt.close()
...
def main() -> None:
    ...
    df_runs.to_csv(results_dir / "multi_run_scmm_v3.csv", index=False)
    ...
    pd.DataFrame([aggregate]).to_csv(results_dir / "multi_run_scmm_v3_summary.csv", index=False)
    save_histogram(df_runs, plots_dir)

    print("\nSCMM-v3 aggregate summary:")
