...
def save_histogram(df_runs: pd.DataFrame, plot_dir: Path) -> None:
    ...
    plt.title("Distribution of Final Wealth Across Inventory + Drawdown SCMM Runs")
    plt.tight_layout()
    plt.savefig(plot_dir / "multi_run_scmm_rule_drawdown_final_wealth_hist.png")
    plt.close()
...
def save_drawdown_histogram(df_runs: pd.DataFrame, plot_dir: Path) -> None:
    ...
    plt.ylabel("Frequency")
    plt.title("Distribution of Max Drawdown Across Inventory + Drawdown SCMM Runs")
    plt.tight_layout()
    plt.savefig(plot_dir / "multi_run_scmm_rule_drawdown_max_drawdown_hist.png")
    plt.close()
...
def main() -> None:
    ...
    df_runs.to_csv(results_dir / "multi_run_scmm_rule_drawdown.csv", index=False)
    ...
    df_agg.to_csv(results_dir / "multi_run_scmm_rule_drawdown_summary.csv", index=False)

    save_histogram(df_runs, plots_dir)
    save_drawdown_histogram(df_runs, plots_dir)

    print("\nPer-run results saved to:")
    print(results_dir / "multi_run_scmm_rule_drawdown.csv")

    print("\nAggregate summary saved to:")
    print(results_dir / "multi_run_scmm_rule_drawdown_summary.csv")
